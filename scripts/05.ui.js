...
ui.action.scores = function () {
  ...
  for (var map in list) {
    ...
    list[map].forEach(function (r) {
      out += '<li>' + '<a>' + new Date(r.date).toDateString() + '</a> ' +
        r.score + ' ☠' + r.kills + ' $' + r.spent + '</li>';
    });
...
ui.action.build = function (type) {
  ...
  var turret = {
    x: -100, // 0,
    y: -100, // 0,
    levels: { range: 0, rate: 0, damage: 0, full: false },
    kills: 0,
    lastshot: 0,
  ...
  for (var k in tdata) {
    turret[k] = tdata[k]; // console.log("turret " + k + ": " + turret[k]);
  }
  game.selection = game.cash - tdata.cost >= 0 ? { status: "placing", turret: turret, placeable: false } : false;
};
...
ui.action.move = function () {
  if (game.selection.status === "selected" && game.cash - 90 >= 0) {
    ...
    game.selection = { status: "moving", turret: turret, placeable: true };

    turret._x = turret.x;
    turret._y = turret.y;

    var tx = (turret._x + 2.5) / 5;
    var ty = (turret._y + 2.5) / 5;
...
$("pages-canvas").addEventListener("click", function (evt) {
  ...
  if (selection.status === "moving") {
    ...
    game.selection = { status: "selected", turret: tile };
